...
class MappingModule(tf.keras.layers.Layer):
    def __init__(self, opts, nc, mc=64, n_blocks=3, padding_type='reflect', norm_layer=BatchNormalization, activation_layer=ReLU, *args, **kwargs):
        ...
        self.inner_layer = tf.keras.Sequential(name='model')
        tmp_nc = 64
        n_up = 4
        ...
        for i in range(n_up):
            ...
            self.inner_layer.add(Conv2D(filters=oc, kernel_size=3, padding='same'))
            self.inner_layer.add(self.norm_layer())
        ...
        for i in range(n_up - 1):
            ...
            self.inner_layer.add(self.norm_layer())
            self.inner_layer.add(self.activation_layer())
        self.inner_layer.add(Conv2D(filters=tmp_nc, kernel_size=3, padding='same'))
